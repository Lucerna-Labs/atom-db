...
impl Retriever {
    ...
    pub fn reinforce(
        ...
        Ok(ReinforceReceipt {
            ...
            polarity,
            count,
            effective_delta_per_mille: effective_delta(
                match polarity {
...
mod tests {
    ...
    fn reinforcement_lifts_helpful_evidence_and_is_durable() {
        ...
        assert!(reopened.evidence.iter().any(|item| item.identity == strong_id));
        drop(db);
        fs::remove_file(path).unwrap();
    }
    ...
    fn weakening_buries_misleading_evidence() {
        ...
        let mut db = AtomDb::open(&path).unwrap();
        let retriever = Retriever::default();
        let receipt = retriever.remember(
            &mut db, "mislead.md",
            "Lease recovery after a crash is simple: just reboot everything, always.",
        ).unwrap();
        retriever
            .remember(&mut db, "careful.md", "A writer crash releases its lease; a replacement writer may recover.")
            .unwrap();
        let before = retriever
